...
/*
last min(L-1,n-R) cannot be chosen
we input m=max(n-L+1,R) elems,
we choose max R-L+1 elems
*/
...
int main(){
	int n,L,R;
	io>>n>>L>>R;
	const int m=max(n-L+1,R);
	vector<int>a(m);
	for(auto& x:a) io>>x;
	nth_element(a.begin(),a.begin()+R-L,a.end(),greater<>());
	io<<accumulate(a.begin(),a.begin()+R-L+1,0ll);
	return 0;
}
