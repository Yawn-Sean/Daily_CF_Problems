...
#if ONLINE_JUDGE
Template690E1.Solution690E1 a = new();
a.Main();
#endif

namespace Template690E1 { 
    internal class Solution690E1 {
        public void Solve() {
            int h = br.ReadInt32(), w = br.ReadInt32();
            int[] t = null, b = null;
            for (int i = 0; i < h; ++i) {
                if (i == 0) t = br.ReadInt32(w);
                else if (i == h - 1) b = br.ReadInt32(w);
                else _ = br.ReadInt32(w);
            }
            bw.AppendYes(t.Zip(b, (a, b) => Math.Abs(a - b)).Sum() <= 10000);
        }
        private readonly BufferedReader br = new(File.OpenRead("all.in"), 1 << 16);
        private readonly BufferedWriter bw = new(File.OpenWrite("all.out"));
        public void Main() {
            int T = 1;
            T = br.ReadInt32();
            while (T-- > 0) {
