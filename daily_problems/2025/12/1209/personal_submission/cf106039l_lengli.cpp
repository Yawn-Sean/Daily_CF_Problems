...
void solve(){
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            if(g[i][j]!='#'){
                a[0][i*n+j]=Mint(g[i][j]-'0');
                f[i*n+j][i*n+j]=1;
                for(int fx=0;fx<8;fx++){
                    int ni=i+dx[fx],nj=j+dy[fx];
                    if(ni<0 or ni>=n or nj<0 or nj>=n or g[ni][nj]=='#') continue;
                    f[i*n+j][ni*n+nj]=1;
                }
            }
        }
