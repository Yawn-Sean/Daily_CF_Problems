...
void solve(){
    ...
    f[2][1]=1;
    std::vector<std::vector<Mint>> pre(n+2,std::vector<Mint> (n+2,0));
    for(int i=1;i<=n;i++) pre[2][i]=1;

    Mint res=0;
    for(int i=3;i<=n;i++){
        for(int j=1;j<=i;j++){
            Mint tmp=0;
            if(a[i]>a[i-1]) tmp=pre[i-1][j];
            else tmp=pre[i-1][j-1];
            f[i][j]=tmp*(i-j>=1 ? p[i-j-1] : 0);
            if(i==n) res+=f[i][j];
