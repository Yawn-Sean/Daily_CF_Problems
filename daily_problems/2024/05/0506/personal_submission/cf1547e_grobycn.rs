...
fn solver(n: usize, k: usize, scanner: &mut Scanner) -> impl Printable {
    ...
    let a: Vec<usize> = scanner.iter(k).collect();
    let mut ac = vec![0; n];
    for (x, t) in a.into_iter().zip(scanner.iter::<i64>(k)) {
        ac[x - 1] = t;
    }
    let mut mn = i64::MAX / 2;
    for (i, (ans, &t)) in ans.iter_mut().zip(ac.iter()).enumerate() {
        let i = i as i64;
        if t > 0 {
            mn = mn.min(t - i);
        }
    ...
    mn = i64::MAX / 2;
    for (i, (ans, &t)) in ans.iter_mut().zip(ac.iter()).enumerate().rev() {
        let i = i as i64;
        if t > 0 {
            mn = mn.min(t + i);
        }
