...
const REM: [&'static str; 7] = ["1869", "1968", "1689", "6198", "1698", "1986", "1896"];

fn solver(a: String) -> impl Printable {
    use std::iter::repeat;
    let mut count = [0; 10];
    a.bytes().for_each(|b| count[(b - b'0') as usize] += 1);
    let mut ans = a.into_bytes();
    [1, 6, 8, 9].iter().for_each(|&x| count[x] -= 1);
    ans.clear();
    count[1..]
        .iter()
        .zip(b'1'..)
        .for_each(|(&c, b)| ans.extend(repeat(b).take(c)));
    let m = ans
        .iter()
        .fold(0, |acc, &b| (10 * acc + (b - b'0') as usize) % 7);
    ans.extend(REM[(7 - (4 * m) % 7) % 7].bytes());
    ans.extend(repeat(b'0').take(count[0]));
