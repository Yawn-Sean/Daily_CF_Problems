...
fn solver(n: usize, k: usize, scanner: &mut Scanner) -> impl Printable {
    ...
    // dp[i][j] means we select i segments, in which j segments has 1 more number
    let mut dp = vec![vec![i64::MAX; r + 1]; k + 1];
    dp[0][0] = 0;
    for i in 1..=k {
        for j in 0..=i.min(r) {
            let len = i * m + j;
            if i > j {
                dp[i][j] = dp[i][j].min(a[len - 1] - a[len - m] + dp[i - 1][j]);
            }
            if j > 0 {
                dp[i][j] = dp[i][j].min(a[len - 1] - a[len - m - 1] + dp[i - 1][j - 1]);
            }
        }
    }
    dp[k][r]
