...
fn solver(n: usize, b: usize, k: usize, x: usize, scanner: &mut Scanner) -> impl Printable {
    let mut count = [0; 10];
    for d in scanner.iter::<usize>(n) {
        count[d] += 1;
    }
    let mut mat = vec![vec![0; x]; x];
    for r in 0..x {
...
fn mat_power(mut base: Vec<Vec<usize>>, mut exp: usize) -> Vec<Vec<usize>> {
    let n = base.len();
    let mut ret = vec![vec![0; n]; n];
    let mut buf = vec![vec![0; n]; n];
    ret.iter_mut().enumerate().for_each(|(r, row)| row[r] = 1);
    while exp > 0 {
        if exp & 1 != 0 {
            mat_mulassign(&mut ret, &base, &mut buf);
        }
        let rhs = base.clone();
        mat_mulassign(&mut base, &rhs, &mut buf);
        exp >>= 1;
    }
    ret
}

fn mat_mulassign(lhs: &mut Vec<Vec<usize>>, rhs: &[Vec<usize>], tmp: &mut Vec<Vec<usize>>) {
    let n = lhs.len();
    for (ret, lhs) in tmp.iter_mut().zip(lhs.iter()) {
        for (c, ret) in ret.iter_mut().enumerate() {
            *ret = 0;
            for (&lhs, rhs) in lhs.iter().zip(rhs.iter().map(|rhs| rhs[c])) {
                *ret = (*ret + lhs * rhs) % MOD;
            }
        }
    }
    std::mem::swap(lhs, tmp);
}
