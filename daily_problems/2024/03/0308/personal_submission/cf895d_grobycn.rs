...
fn power(mut e: usize, mut x: usize) -> usize {
    let mut ret = 1;
    while x > 0 {
        if x & 1 != 0 {
            ret = (ret * e) % MOD;
...
fn solver(a: String, b: String) -> impl Printable {
    let n = a.len();
    ...
    let mut count = [0; 26];
    a.bytes().for_each(|b| count[(b - b'a') as usize] += 1);

    fn count_perm(max: &[u8], mut cnt: [usize; 26], fac: &[usize], facinv: &[usize]) -> usize {
        let n = max.len();
        let mut ret = 0;
        // 1/(c1!c2!c3!...)
        let mut cur = cnt.iter().fold(1, |acc, &c| (acc * facinv[c]) % MOD);

        for (i, &b) in max.iter().enumerate() {
            for (_, &c) in (b'a'..b).zip(&cnt) {
                if c > 0 {
                    ret = (ret + ((cur * c) % MOD) * fac[n - i - 1]) % MOD;
                }
            }
    ...
    let mut ret = count_perm(b.as_bytes(), count, &fac, &facinv);
    ret = (ret + MOD - 1 - count_perm(a.as_bytes(), count, &fac, &facinv)) % MOD;
    ret
}
