...
fn solver(b: Vec<i64>) -> impl Printable {
    let n = b.len();
    if n <= 3 {
        return 1;
    }
    let mut indices: Vec<_> = (0..n).collect();
    indices.sort_unstable_by_key(|&i| b[i]);

    for del in 0..=1 {
        let d = b[indices[2]] - b[indices[1 - del]];
        if indices[2..].windows(2).all(|w| b[w[1]] - b[w[0]] == d) {
            return indices[del] as i64 + 1;
        }
    }
    ...
    let mut del = 0;
    let mut cnt = 0;
    for (x, i) in indices[2..].iter().map(|&i| b[i]).zip(2..) {
        if x != a + (i - cnt) * d {
            cnt += 1;
            del = i as usize;
        }
