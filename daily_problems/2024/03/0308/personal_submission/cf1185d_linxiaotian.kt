...
private fun PrintWriter.solve() {
    ...
    println(extraElement(nums))
}

private fun extraElement(nums1: IntArray): Int {
    val n = nums1.size
    if (n <= 3) {
        return 1
    }
    val nums = nums1.sorted()
    fun mappingIndex(i: Int): Int {
        return nums1.indexOf(nums[i]) + 1
    }
    for (index in 0 until 2) {
        if (check((0 until n).filter { it != index }.map { nums[it] })) {
    ...
    return if (deleted == -1) 0 else mappingIndex(deleted)
}

private fun check(nums: List<Int>): Boolean {
    val n = nums.size
    val diff = nums[1] - nums[0]
    return (2 until n).all { nums[it] - nums[it - 1] == diff }
}
