...
private fun PrintWriter.solve() {
    val n = readInt()
    val lines = Array(n * 2) { readLn() }
    val result = shurikens(lines)
    println(result.isNotEmpty().toYesNo(true))
    printNums(result)
}
...
//     }
//     val n = lines.size / 2
//     val pricePosSet = MultiTreeSet<Int>().apply {
//         add(-1)
//     }
//     val priceToPos = IntArray(n + 1)
//     var pos = -1
//     for (line in lines) {
//         if (line[0] == '+') {
//             pos++
//         } else {
//             val price = line.split(" ")[1].toInt()
//             priceToPos[price] = pos
//             pricePosSet.add(pos)
//         }
//     }
//     // println(pricePosSet)
//     // println(priceToPos.toList())
//     val remainPos = TreeSet<Int>().apply {
//         addAll((0 until n).toList())
//     }
//     val result = IntArray(n)
//     for (price in 1 until n + 1) {
//         val pos = priceToPos[price]
//         val prePos = pricePosSet.lower(pos)!!
//         // println("$price | $pos, $prePos | ${remainPos.higher(prePos)}")
//         val currentPos = remainPos.higher(prePos) ?: return emptyList()
//         if (currentPos > pos) {
//             return emptyList()
//         }
//         result[currentPos] = price
//         remainPos.remove(currentPos)
//         pricePosSet.remove(pos)
//         // println("${result.toList()} | $remainPos | $pricePosSet")
//     }
//     return if (check(lines, result)) result.toList() else emptyList()
// }
...
// val queueMax = MultiTreeSet<Int>(compareBy { -it })
class MultiTreeSet<T>(comparator: Comparator<in T>? = null) {

    private val map: TreeMap<T, Int> = if (comparator == null) TreeMap() else TreeMap(comparator)
    var size = 0
        private set
    ...
    fun pollOrNull(): T? {
        val entry = map.pollFirstEntry() ?: return null
        if (entry.value != 1) {
            map[entry.key] = entry.value - 1
        }
        size--
    ...
    fun remove(element: T): Boolean {
        val count = map[element] ?: return false
        if (count > 1) {
            map[element] = count - 1
        } else {
            map.remove(element)
