...
private val TEST_MAX = 10
private val testRect = Array(TEST_MAX + 1) { BooleanArray(TEST_MAX + 1) }
private var testAns = emptyList<Int>()

private fun test() {
    testMode = true
    for (x in 2 until TEST_MAX) {
        for (y in 2 until TEST_MAX) {
            for (p in x until TEST_MAX) {
                for (q in y until TEST_MAX) {
                    testAns = listOf(x, y, p, q)
                    testRect.forEach { it.fill(false) }
                    for (i in x until p + 1) {
                        for (j in y until q + 1) {
                            testRect[i][j] = true
                        }
                    }
                    hiddenFortress(TEST_MAX)
                }
            }
        }
    }
...
private val MAX = 1_000_000_000

private fun hiddenFortress(max: Int) {
    val leftBottom = ask(1, 1)
    val rightBottom = ask(max, 1)
    val leftUp = ask(1, max)
    val xMid = (leftBottom - rightBottom + max + 1) / 2
    val y = ask(xMid, 1) + 1
    val x = leftBottom - y + 2
    val p = y - 1 - rightBottom + max
    val q = leftBottom - y + 1 - leftUp + max
    answer(x, y, p, q)
    return
}

private fun ask(x: Int, y: Int): Int {
    if (testMode) {
        if (testRect[x][y]) {
            println("###1 point in rect")
            return -1
        }
        var dis = maxValue
        for (i in 2 until TEST_MAX) {
            for (j in 2 until TEST_MAX) {
                if (testRect[i][j]) {
                    dis = dis.min(abs(i - x) + abs(j - y))
                }
            }
        }
        return dis
...
private fun answer(x: Int, y: Int, p: Int, q: Int) {
    if (testMode) {
        val current = listOf(x, y, p, q)
        if (current != testAns) {
            println("###2 exp: $testAns | act: $current")
        }
        return
    }
    writer.println("! $x $y $p $q")
    writer.flush()
}
