...
private fun PrintWriter.solve() {
    ...
    println(functionsAgain(nums))
}

private fun functionsAgain(nums: LongArray): Long {
    val n = nums.size
    var result = Long.MIN_VALUE / 2L
    var dp1 = Long.MIN_VALUE / 2L
    var dp2 = Long.MIN_VALUE / 2L
    for (i in 0 until n - 1) {
        val diff = abs(nums[i + 1] - nums[i])
        var ndp1 = max(dp2, 0L) + diff
        var ndp2 = max(dp1, 0L) - diff
        dp1 = ndp1
        dp2 = ndp2
        result = result.max(dp1).max(dp2)
    }
    return result
}
...
// private fun functionsAgain(nums: LongArray): Long {
//     val n = nums.size
//     val diffs = (0 until n - 1).map { abs(nums[it + 1] - nums[it]) }
//     var result = Long.MIN_VALUE / 2L
//     for (start in n - 2 downTo n - 3) {
//         var last = 0L
//         if (start == n - 3) {
//             last = last.max(-diffs[start + 1])
//         }
//         for (i in start downTo 0 step 2) {
//             val current = diffs[i] + last.max(0L)
