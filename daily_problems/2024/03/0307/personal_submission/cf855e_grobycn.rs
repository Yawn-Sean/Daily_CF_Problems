...
fn solver(q: usize, scanner: &mut Scanner) -> impl Printable {
    ...
    fn digits(mut x: usize, base: usize) -> Vec<usize> {
        let mut ret = vec![];
        while x > 0 {
            ret.push(x % base);
            x /= base;
    ...
    let count = (2..=10)
        .map(|b| {
            let len = MAX.ilog(b) as usize + 1;
            let mut count = vec![vec![0; b + 1]; len + 1];
            count[0][0] = 1;
            for l in 1..=len {
                count[l][0] += count[l - 1][1];
                for i in 1..b {
                    count[l][i] +=
                        count[l - 1][i - 1] * (b - (i - 1)) + count[l - 1][i + 1] * (i + 1);
                }
                count[l][b] += count[l - 1][b - 1];
            }
            count
        })
        .collect::<Vec<_>>();
    ...
    let comb = {
        ...
        move |n: usize, k: usize| fac[n] / fac[k] / fac[n - k]
    };

    let count = |max: usize, b: usize| {
        let ds = digits(max, b);
        let mut ret = 0;
        for l in 1..ds.len() {
            ret += count[b - 2][l][0] - count[b - 2][l - 1][1] / b;
        }
        let mut mask = 0;
        let mut bits = 0;
        for (l, &upper) in ds.iter().enumerate().rev() {
            for d in 0..upper {
                if l + 1 == ds.len() && d == 0 {
                    continue;
                }
                let bits = if mask & (1 << d) == 0 {
                    bits + 1
                } else {
                    bits - 1
                };
                ret += count[b - 2][l][bits] / comb(b, bits);
            }
            bits = if mask & (1 << upper) == 0 {
                bits + 1
            } else {
                bits - 1
            };
    ...
    let ans = scanner
        .iter(q)
        .map(|(b, l, r): (usize, usize, usize)| count(r + 1, b) - count(l, b))
        .collect::<Vec<_>>();
    Sep::<_, '\n'>(ans)
}
