...
fn solver(n: usize, scanner: &mut Scanner) -> impl Printable {
    ...
    for (i, k) in scanner.iter::<usize>(n).enumerate() {
        bucket[k].push(i);
    }
    ...
    let mut min_at = usize::MAX;
    let mut min_deg = usize::MAX;
    let mut max_delta = 0;
    let mut ans = (0, 0);
    for (deg, indices) in bucket.iter().enumerate() {
        for &idx in indices {
            cnt += 1;
            sum += deg;
            ...
            if sum == cnt * (cnt - 1) / 2 {
                if idx != min_at && deg - min_deg >= max_delta {
                    max_delta = deg - min_deg;
                    ans = (min_at + 1, idx + 1);
                }
                min_deg = usize::MAX;
                min_at = usize::MAX;
