...
fn solver(w: Vec<i64>, scanner: &mut Scanner) -> impl Printable {
    let n = w.len();
    let mut adj = vec![vec![]; n];
    for (u, v, c) in scanner.iter::<(usize, usize, i64)>(n - 1) {
        adj[u - 1].push((v - 1, c));
        adj[v - 1].push((u - 1, c));
    }

    fn dfs(adj: &[Vec<(usize, i64)>], cur: usize, fa: usize, w: &[i64], ret: &mut i64) -> i64 {
        let mut max = w[cur];
        *ret = max.max(*ret);
        for &(next, c) in &adj[cur] {
            ...
            let subtree_max = dfs(adj, next, cur, w, ret);
            *ret = (max - c + subtree_max).max(*ret);
            max = max.max(subtree_max + w[cur] - c);
        }
        max
    }

    let mut ret = 0;
    dfs(&adj, 0, 0, &w, &mut ret);
    ret
}
