...
fn solver(mut points: Vec<(i32, i32)>) -> impl Printable {
    use std::collections::BTreeMap;
    points.sort_unstable_by_key(|p| (-p.1, p.0));
    let mut mp = BTreeMap::new();
    points.iter().for_each(|&(x, _)| {
        mp.insert(x, 0);
    });
    mp.values_mut().enumerate().for_each(|(i, v)| *v = i);
    let m = mp.len();
    let mut tree = Fenwick::new(m);
    let mut has = vec![false; m];
    let mut cnt = 0;
    let mut ret = 0i64;
    ...
    while s < points.len() {
        let mut e = s + 1;
        while e < points.len() && points[e].1 == points[s].1 {
            e += 1;
        }
        let mut l = 0;
        for (x, _) in &points[s..e] {
            let idx = mp[x];
            if !has[idx] {
                has[idx] = true;
                tree.inc(idx, 1);
            ...
            let c = tree.query(l, idx);
            ret -= (c + 1) * c / 2;
            l = idx + 1;
        }
        ret += (cnt + 1) * cnt / 2;
        let c = tree.query(l, m);
        ret -= (c + 1) * c / 2;
        s = e;
    }
    ret
...
impl Fenwick {
    ...
    fn inc(&mut self, mut idx: usize, delta: i64) {
        idx += 1;
        while idx <= self.arr.len() {
            self.arr[idx - 1] += delta;
            idx += idx & !(idx - 1);
        }
    }
}
