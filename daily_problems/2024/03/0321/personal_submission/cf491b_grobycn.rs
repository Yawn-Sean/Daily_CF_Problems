...
fn solver(n: usize, m: usize, scanner: &mut Scanner) -> impl Printable {
    let [mut maxpp, mut maxpm, mut maxmp, mut maxmm] = [i64::MIN; 4];
    let c = usize::from_scanner(scanner);
    for (x, y) in scanner.iter::<(i64, i64)>(c) {
        maxpp = maxpp.max(x + y);
        maxpm = maxpm.max(x - y);
        maxmp = maxmp.max(-x + y);
        maxmm = maxmm.max(-x - y);
    }
    let h = usize::from_scanner(scanner);
    let mut mn = i64::MAX;
    let mut mn_at = 0;
    for ((x, y), i) in scanner.iter::<(i64, i64)>(h).zip(1..) {
        let mx = (maxpp - (x + y))
            .max(maxpm - (x - y))
            .max(maxmp - (-x + y))
            .max(maxmm - (-x - y));
