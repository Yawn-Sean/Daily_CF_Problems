...
fn solver(n: usize, mut l: usize, scanner: &mut Scanner) -> impl Printable {
    let mut cs: Vec<usize> = scanner.iter(n).collect();
    for i in 1..n {
        cs[i] = cs[i].min(2 * cs[i - 1]);
    }
    let mut ans = usize::MAX;
    let mut cost = 0;
    let mut v = 1 << (n - 1);
    for &c in cs.iter().rev() {
        ans = ans.min(cost + (l + v - 1) / v * c);
        cost += l / v * c;
        l %= v;
        v >>= 1;
    }
