...
private fun PrintWriter.solve() {
    readInts()
    val c = readInt()
    val hotels = Array(c) { readLongArray() }
    val h = readInt()
    val restaurants = Array(h) { readLongArray() }
    newYorkHotel(hotels, restaurants).forEach { println(it) }
}

private fun newYorkHotel(hotels: Array<LongArray>, restaurants: Array<LongArray>): List<String> {
    var maxXPlusy = -maxValueL
    var minXPlusy = maxValueL
    var maxXMinusy = -maxValueL
    var minXMinusy = maxValueL
    for ((x, y) in hotels) {
        maxXPlusy = max(maxXPlusy, x + y)
        minXPlusy = min(minXPlusy, x + y)
        maxXMinusy = max(maxXMinusy, x - y)
        minXMinusy = min(minXMinusy, x - y)
    }
    var value = maxValueL
    var index = -1
    for (i in 0 until restaurants.size) {
        val (x, y) = restaurants[i]
        val distance = maxOf(
            x + y - minXPlusy,
            -x - y + maxXPlusy,
            y - x + maxXMinusy,
    ...
    return listOf(value, index).map { it.toString() }
}
