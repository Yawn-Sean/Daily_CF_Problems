...
private fun PrintWriter.solve() {
    val n = readInt()
    val parents = readIntArray1(n - 1)
    val sums = readLongArray(n)
    println(sumInTheTree(parents, sums))
}

private fun sumInTheTree(parents: IntArray, sums: LongArray): Long {
    val n = parents.size + 1
    val graph = Array(n) { mutableListOf<Int>() }
    for (i in 1 until n) {
        val p = parents[i - 1]
        graph[p].add(i)
    ...
    val nodes = LongArray(n)
    // search(0, 1, 0L, sums, nodes, graph)
    // println(nodes.toList())
    return if (search(0, 1, 0L, sums, nodes, graph)) nodes.sum() else -1L
}
...
private fun search(
    ...
    if (sums[node] >= 0L && sums[node] < pathSum) {
        return false
    }
    if (depth % 2 == 0) {
        var min = maxValueL
        for (next in graph[node]) {
            min = min.min(sums[next])
        }
        // println("$node, $depth | $max")
        if (min != maxValueL) {
            if (min < pathSum) {
                return false
            }
            nodes[node] = min - pathSum
        }
    } else {
        nodes[node] = sums[node] - pathSum
    }
    for (next in graph[node]) {
        if (!search(next, depth + 1, pathSum + nodes[node], sums, nodes, graph)) {
            return false
        }
    }
    return true
