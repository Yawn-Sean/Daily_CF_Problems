...
private fun PrintWriter.solve() {
    val (m, n) = readInts()
    val grid = Array(m) { readIntArray(n) }
    theTable(grid).forEach { nums ->
        print(nums.size)
        print(' ')
        printNums(nums)
    }
...
private fun theTable(grid: Array<IntArray>): List<List<Int>> {
    val (m, n) = grid.size to grid[0].size
    val rowSums = IntArray(m)
    ...
    while (true) {
        val row = rowSums.indexOfFirst { it < 0 }
        if (row >= 0) {
            for (j in 0 until n) {
                rowSums[row] -= 2 * grid[row][j]
                colSums[j] -= 2 * grid[row][j]
                grid[row][j] = -grid[row][j]
            }
            rowCounts[row]++
            continue
        }
        val col = colSums.indexOfFirst { it < 0 }
        if (col >= 0) {
            for (i in 0 until m) {
                colSums[col] -= 2 * grid[i][col]
                rowSums[i] -= 2 * grid[i][col]
    ...
    val rows = (0 until m).filter { rowCounts[it] % 2 == 1 }
    val cols = (0 until n).filter { colCounts[it] % 2 == 1 }
    return listOf(rows.map { it + 1 }, cols.map { it + 1 })
}
