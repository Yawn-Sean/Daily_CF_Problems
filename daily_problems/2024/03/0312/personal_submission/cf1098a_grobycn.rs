...
fn solver(n: usize, scanner: &mut Scanner) -> impl Printable {
    ...
    ps.push(0);
    ps.extend(scanner.iter(n - 1).map(|x: usize| x - 1));
    let mut ss: Vec<i64> = scanner.iter(n).collect();
    ...
    for (i, (&p, &h)) in ps.iter().zip(&hs).enumerate().rev() {
        if h % 2 == 0 {
            if ss[ps[p]] > ss[i] {
                return -1;
            }
    ...
    ss[0] + ps.iter().zip(&ss).map(|(&p, &s)| s - ss[p]).sum::<i64>()
}
