...
fn solver(Mat(mut mat): Mat<i64>) -> impl Printable {
    let (m, n) = (mat.len(), mat[0].len());
    let mut rsum: Vec<i64> = mat.iter().map(|row| row.iter().sum()).collect();
    let mut csum: Vec<i64> = (0..n).map(|c| mat.iter().map(|row| row[c]).sum()).collect();
    let is_done = |rsum: &[i64], csum: &[i64]| rsum.iter().chain(csum).all(|&x| x >= 0);
    let flip = |(x, sum): (&mut i64, &mut i64)| {
        *sum -= *x * 2;
        *x *= -1;
    };

    let (mut rsp, mut csp) = (vec![0u8; m], vec![0u8; n]);
    while !is_done(&rsum, &csum) {
        for (r, (sum, row)) in rsum.iter_mut().zip(mat.iter_mut()).enumerate() {
            if *sum < 0 {
                rsp[r] ^= 1;
                *sum *= -1;
                row.iter_mut().zip(&mut csum).for_each(flip);
            }
        }
        for (c, sum) in csum.iter_mut().enumerate() {
            if *sum < 0 {
                csp[c] ^= 1;
                *sum *= -1;
                mat.iter_mut().map(|row| &mut row[c]).zip(&mut rsum).for_each(flip);
            }
        }
    }

    let rsp = (1..=m).filter(|&i| rsp[i - 1] == 1).collect::<Vec<_>>();
    let csp = (1..=n).filter(|&i| csp[i - 1] == 1).collect::<Vec<_>>();
    (rsp.len(), Sep::<_, ' '>(rsp))
        .cr()
        .then((csp.len(), Sep::<_, ' '>(csp)))
}
