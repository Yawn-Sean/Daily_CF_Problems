...
fn solver(n: usize, m: usize, k: i32, scanner: &mut Scanner) -> impl Printable {
    ...
    fn dfs(
        ...
        for &next in &adj[cur] {
            if dist[next] == -1 {
                ...
                if let Some(ans) = dfs(adj, next, k, dist, from) {
                    return Some(ans);
                }
            } else if dist[cur] - dist[next] >= k {
                let mut idx = cur;
                let mut ans = vec![idx + 1];
                while idx != next {
                    idx = from[idx];
                    ans.push(idx + 1);
                }
    ...
    let ans = dfs(&adj, 0, k, &mut dist, &mut from).unwrap();
