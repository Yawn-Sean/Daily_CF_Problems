...
static ONCE: OnceLock<[usize; MAX + 1]> = OnceLock::new();
...
impl<'a> Factors<'a> {
    fn new(lpf: &'a [usize], x: usize) -> Self {
        Self { lpf, x }
    }
}
...
fn solver(mut n: usize, q: usize, scanner: &mut Scanner) -> impl Printable {
    let lpf = ONCE.get_or_init(|| {
        let mut primes = vec![];
        let mut lpf = [0; MAX + 1];
        for i in 2..=MAX {
            if lpf[i] == 0 {
    ...
    let mut factors: HashMap<_, _> = Factors::new(lpf, n).collect();
    let origin_factors = factors.clone();

    let mut ans = vec![];
    for _ in 0..q {
        if u8::from_scanner(scanner) == 1 {
            let x = usize::from_scanner(scanner);
            Factors::new(lpf, x).for_each(|(p, cnt)| *factors.entry(p).or_insert(0) += cnt);
            // dn = factors.values().map(|&c| c + 1).product()
            let mut dn = HashMap::new();
            factors.values().for_each(|&c| {
                Factors::new(lpf, c + 1).for_each(|(p, cnt)| *dn.entry(p).or_insert(0) += cnt)
            });
            if dn
                .into_iter()
                .all(|(p, cnt)| matches!(factors.get(&p), Some(&c) if c >= cnt))
            {
                ans.push("YES");
            } else {
                ans.push("NO");
