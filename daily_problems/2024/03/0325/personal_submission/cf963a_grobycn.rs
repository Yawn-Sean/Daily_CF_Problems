...
fn solver(n: usize, a: usize, b: usize, k: usize, signs: String) -> impl Printable {
    let mut ans: Mod = Mod(0);
    let q = Mod(b) / Mod(a);
    let qk = q.power(k);
    let mut sum = if qk.0 == 1 {
        Mod(a).power(n) * Mod((n + 1) / k)
    } else {
        Mod(a).power(n) * (Mod(1) - qk.power((n + 1) / k)) / (Mod(1) - qk)
    };

    for s in signs.chars() {
        if s == '+' {
            ans = ans + sum;
        } else {
            ans = ans - sum;
...
#[derive(Clone, Copy)]
struct Mod<const M: usize = 1_000_000_009>(usize);

use std::ops::{Add, Div, Mul, Sub};

impl<const M: usize> Mod<M> {
    pub fn power(self, mut exp: usize) -> Self {
        let mut ret = Mod(1);
        let mut base = self;
        while exp > 0 {
            if exp & 1 != 0 {
...
impl<const M: usize> Div for Mod<M> {
    type Output = Self;
