...
private fun PrintWriter.solve() {
    val (n, a, b, k) = readInts(4)
    val s = readString()
    println(alternatingSum(n, a.toLong(), b.toLong(), k, s))
}

private fun alternatingSum(n: Int, a: Long, b: Long, k: Int, s: String): Long {
    var result = 0L
    val invA = inv(a)
    val kl = k.toLong()
    val q = fastPow(invA, kl).modMul(fastPow(b, kl))
    val invQ1 = inv(q.modSum(-1L))
    var head = fastPow(a, n.toLong())
    for (i in 0 until k) {
        val len = n.toLong() / k + (if (i <= n % k) 1 else 0)
        val mul = if (q == 1L) {
            len
        } else {
            fastPow(q, len).modSum(-1L).modMul(invQ1)
        }
        var current = head.modMul(mul)
        if (s[i] == '-') {
            current = current.modMul(-1L)
        }
        // println("$i | $head, $len $current")
        result = result.modSum(current)
        head = head.modMul(invA).modMul(b)
    }
    return result
}
...
//     val q = fastPow(invA, kl).modMul(fastPow(b, kl))
//     val m = n / k + 1
//     val muls = LongArray(m + 1) { it.toLong() }
//     if (q != 1L) {
//         val invQ1 = inv(q.modSum(-1L))
//         var qq = 1L
//         for (len in 1 until m + 1) {
//             qq = qq.modMul(q)
//             muls[len] = qq.modSum(-1L).modMul(invQ1)
//         }
//     }
//     // println(q)
//     // println(muls.toList())
...
//     var head = fastPow(a, n.toLong())
//     for (i in 0 until k) {
//         val len = n / k + (if (i <= n % k) 1 else 0)
//         var current = head.modMul(muls[len])
//         if (s[i] == '-') {
//             current = current.modMul(-1L)
//         }
//         // println("$i | $head, $len $current")
