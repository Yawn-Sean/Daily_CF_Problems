...
private fun PrintWriter.solve() {
    ...
    println(if (petrPermutations(nums)) "Petr" else "Um_nik")
...
private fun test() {
    ...
    repeat(100) {
        ...
        // counts1.add((0 until n).count { it == nums1[it] })
        counts1.add(inversionCount(nums1))
        val nums2 = IntArray(n) { it }
        repeat(n * 7 + 1) {
            swapRandom(nums2)
        }
        // counts2.add((0 until n).count { it == nums2[it] })
        counts2.add(inversionCount(nums2))
    }
    println(counts1)
    println(counts2)
    println("fin")
...
private fun inversionCount(nums: IntArray): Long {
    val n = nums.size
    // val nums = unify(nums1)
    val bit = BinaryIndexedTree(n + 1)
    var result = 0L
    for (i in 0 until n) {
        result += i - bit.query(nums[i])
        bit.add(nums[i], 1)
    }
    return result
}

private fun petrPermutations(nums: IntArray): Boolean {
    val count = inversionCount(nums)
    return (count - nums.size.toLong()) % 2L == 0L
}

// WA
// private fun petrPermutations(nums: IntArray): Boolean {
//     val n = nums.size
//     val count = (0 until n).count { nums[it] == it + 1 }
//     val thre = if (n == 1000) 1 else 2
//     return count <= thre
// }
