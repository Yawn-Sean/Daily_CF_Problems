/**
 * Created by 5cm/s on 2024/03/25 11:00:55.
 * 诸天神佛，佑我上分！
 **/
#include <bits/stdc++.h>
...
void elysia() {
    ...
    a = power(a, k), b = power(b, k);
    map<int, Mint> f;
    f[1] = p;
    function<Mint(int)> dfs = [&](int n) -> Mint {
        if (f.contains(n)) return f[n];
        int l = n / 2, r = n - l;
        return f[n] = dfs(l) * power(a, r) + dfs(r) * power(b, l);
    };
