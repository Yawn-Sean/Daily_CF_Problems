...
fn solver(n: usize, scanner: &mut Scanner) -> impl Printable {
    const AMPLIFIER: i128 = 1_000_000;
    let xs = scanner
        .iter::<i128>(n)
        .map(|x| x * AMPLIFIER)
        .collect::<Vec<_>>();
    let vs = scanner.iter::<i128>(n).collect::<Vec<_>>();
    let check = |t: i128| {
        let mut left = i128::MIN;
        let mut right = i128::MAX;
        for (&x, &v) in xs.iter().zip(&vs) {
            left = left.max(x - v * t);
            right = right.min(x + v * t);
        }
        left <= right
    };

    let (mut lo, mut hi) = (1, 1_000_000_000 * AMPLIFIER);
    while lo < hi {
        let mid = (hi - lo) / 2 + lo;
        if check(mid) {
            hi = mid;
    ...
    format!("{}.{:06}", hi / AMPLIFIER, hi % AMPLIFIER)
}
