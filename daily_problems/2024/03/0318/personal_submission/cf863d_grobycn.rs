...
fn solver(n: usize, q: usize, m: usize, scanner: &mut Scanner) -> impl Printable {
    let a: Vec<i32> = scanner.iter(n).collect();
    let q: Vec<(u8, usize, usize)> = scanner.iter(q).collect();
    let ans = scanner
        .iter(m)
        .map(|mut i: usize| {
            for &(t, l, r) in q.iter().rev() {
                if i < l || r < i {
                    continue;
                }
                if t == 1 {
