...
fn solver(points: Vec<(i64, i64)>) -> impl Printable {
    fn max_dist((x0, y0): (i64, i64), (x1, y1): (i64, i64), (x2, y2): (i64, i64)) -> f64 {
        let (dx01, dy01) = (x1 - x0, y1 - y0);
        let (dx02, dy02) = (x2 - x0, y2 - y0);
        let area2 = dx02 * dy01 - dy02 * dx01;
        let len2 = dx02 * dx02 + dy02 * dy02;
        (area2 as f64) / (len2 as f64).sqrt() / 2.0
    }
    let mut ans = f64::MAX;
    let n = points.len();
    for i in 0..n {
        ans = ans.min(max_dist(
            points[i],
            points[(i + 1) % n],
            points[(i + 2) % n],
        ));
