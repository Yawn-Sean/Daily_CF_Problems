...
private fun cliqueGraph(nums: List<Int>): Int {
    ...
    val dp = counts.copyOf()
    for (num in 1 until max + 1) {
        if (counts[num] == 0) {
            continue
        }
        for (next in num + num until max + 1 step num) {
            dp[next] = dp[next].max(dp[num] + counts[next])
        }
    }
    // println(numToLen)
...
//     val n = nums.size
//     val max = nums.findMax()
//     val numToLen = nums.map { it to 1 }.toMap().toMutableMap()
//     for (num in nums.sorted()) {
//         val len = numToLen[num]!! + 1
//         for (next in num + num until max + 1 step num) {
//             if (next in numToLen) {
//                 numToLen[next] = numToLen[next]!!.max(len)
//             }
//         }
//     }
//     // println(numToLen)
//     return numToLen.values.findMax()
// }
