...
private fun PrintWriter.solve() {
    val n = readInt()
    val xList = IntArray(n)
    val yList = IntArray(n)
    for (i in 0 until n) {
        xList[i] = readInt()
        yList[i] = readInt()
    }
    println(numberOfParalograms(xList, yList))
}

private fun numberOfParalograms(xList: IntArray, yList: IntArray): Long {
    val n = xList.size
    val counter = Counter<Long>()
    val rand = Random(123L).nextInt().toLong()
    var result = 0L
    for (i in 0 until n) {
        for (j in 0 until i) {
            val x = xList[i] + xList[j]
            val y = yList[i] + yList[j]
            val key = x * rand + y
...
// private fun PrintWriter.solve() {
//     val n = readInt()
//     val pointList = Array(n) { readIntArray() }
//     println(numberOfParalograms(pointList))
// }
// private fun numberOfParalograms(pointList1: Array<IntArray>): Long {
//     val n = pointList1.size
//     val pointList = pointList1.sortedBy { it[0] }
//     val counter1 = Counter<Long>()
//     val counter2 = Counter<Long>()
//     for (i in 0 until n) {
//         for (j in i + 1 until n) {
//             val dx = pointList[j][0] - pointList[i][0]
//             val dy = pointList[j][1] - pointList[i][1]
//             if (dy >= 0) {
//                 counter1.add(makePoint(dx, dy))
//             } else {
//                 counter2.add(makePoint(dx, -dy))
//             }
//         }
//     }
//     // println(counter1.numToCount)
//     // println(counter2.numToCount)
//     var result = 0L
//     for (count in counter1.numToCount.values) {
//         result += count * (count - 1) / 2
//     }
//     for (count in counter2.numToCount.values) {
//         result += count * (count - 1) / 2
//     }
//     return result / 2L
// }
...
//     val n = pointList1.size
//     val pointList = pointList1.sortedBy { it[0] }
//     val rand = Random(1234L).nextInt().toLong()
//     val counter = Counter<Long>()
//     for (i in 0 until n) {
//         for (j in i + 1 until n) {
//             val dx = pointList[j][0] - pointList[i][0]
...
//         }
//     }
//     // println(counter.numToCount)
//     var result = 0L
//     for (count in counter.numToCount.values) {
//         result += count * (count - 1) / 2
//     }
//     return result / 2L
// }
