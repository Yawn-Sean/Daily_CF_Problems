...
private fun edgesToGraph(n: Int, edges: Array<IntArray>, directed: Boolean = false): Graph {
...
private fun PrintWriter.printNums(nums: LongArray) {
    if (nums.isEmpty()) {
        println()
        return
    }
...
private fun PrintWriter.solve() {
    val n = readInt()
    val edges = Array(n - 1) { readIntArray1() }
    val q = readInt()
    val queryList = Array(q) { readIntArray(3) }
    printNums(vasyaTree(edges, queryList))
}

private fun vasyaTree(edges: Array<IntArray>, queryList: Array<IntArray>): LongArray {
    val n = edges.size + 1
    val graph = edgesToGraph(n, edges)
    val nodeInfos = Array(n) { mutableListOf<IntArray>() }
    for (query in queryList) {
        val node = query[0] - 1
        nodeInfos[node].add(intArrayOf(query[1], query[2]))
    }
    return LongArray(n).apply {
        search(0, -1, 0, graph, nodeInfos, BinaryIndexedTreeDiff(n + 100), this)
    }
}
...
private fun search(
    ...
    depth: Int,
    graph: Graph,
    nodeInfos: Array<MutableList<IntArray>>,
    bit: BinaryIndexedTreeDiff,
    result: LongArray,
) {
    val n = graph.size
    fun addToBit(mul: Long) {
        for ((d, x) in nodeInfos[node]) {
            bit.add(depth, min(n, depth + d), x * mul)
        }
    }
    addToBit(1L)
    result[node] = bit.query(depth)
    for (next in graph[node]) {
        if (next == parent) {
            continue
        }
        search(next, node, depth + 1, graph, nodeInfos, bit, result)
    }
    addToBit(-1L)
}

class BinaryIndexedTreeDiff {

    private val bit: BinaryIndexedTreeLong

    constructor(n: Int) {
        this.bit = BinaryIndexedTreeLong(n + 1)
    }

    fun query(index: Int): Long {
        return bit.query(index)
    }

    fun add(left: Int, right: Int, x: Long = 1) {
        bit.add(left, x)
        bit.add(right + 1, -x)
    }
}
