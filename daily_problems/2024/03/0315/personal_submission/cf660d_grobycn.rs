...
fn solver(points: Vec<(usize, usize)>) -> impl Printable {
    let mut count = std::collections::HashMap::new();
    for (i, &(x0, y0)) in points.iter().enumerate() {
        for &(x1, y1) in &points[..i] {
            let xmid2 = x0 + x1; // xmid = (x0 + x1) / 2
            let ymid2 = y0 + y1;
            *count.entry((xmid2 << 32) | ymid2).or_insert(0) += 1;
        }
    }
    count.values().map(|&c| c * (c - 1) / 2).sum::<usize>()
}
