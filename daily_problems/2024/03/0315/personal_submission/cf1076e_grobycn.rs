...
fn solver(n: usize, scanner: &mut Scanner) -> impl Printable {
    ...
    for (v, d, x) in scanner.iter::<(usize, usize, i64)>(m) {
        ops[v - 1].push((d, x));
    }

    enum Operation {
        DFS(usize, usize, usize, i64), // index, parent, depth, sum
        RESTORE(usize, usize),         // index, depth
    }
    let mut diff = vec![0; n];
    let mut ans = vec![0; n];
    let mut stack = vec![Operation::DFS(0, 0, 0, 0)];
    while let Some(op) = stack.pop() {
        match op {
            Operation::DFS(cur, fa, mut dep, mut sum) => {
                for &(d, x) in &ops[cur] {
                    diff[dep] += x;
                    if dep + d + 1 < diff.len() {
                        diff[dep + d + 1] -= x;
                ...
                stack.push(Operation::RESTORE(cur, dep));
                sum += diff[dep];
                ans[cur] = sum;
                dep += 1;
                for &next in &adj[cur] {
                    ...
                    stack.push(Operation::DFS(next, cur, dep, sum));
                }
            }
            Operation::RESTORE(cur, dep) => {
                for &(d, x) in &ops[cur] {
                    diff[dep] -= x;
                    if dep + d + 1 < diff.len() {
                        diff[dep + d + 1] += x;
