...
fn solver(polya: Vec<(i64, i64)>, polyb: Vec<(i64, i64)>) -> impl Printable {
    let (mut xmin, mut xmax) = (i64::MAX, i64::MIN);
    for &(x, _) in &polya {
        xmin = xmin.min(x);
        xmax = xmax.max(x);
    }
    let (mut xmin_at, mut xmax_at) = (0, 0);
    for (i, &(x, _)) in polya.iter().enumerate() {
        if x == xmin {
            xmin_at = i;
        }
        if x == xmax {
    ...
    let upper_half: Vec<_> = if xmin_at < xmax_at {
        polya[xmin_at..=xmax_at].iter().copied().collect()
    } else {
        polya[xmin_at..]
            .iter()
            .chain(polya[..=xmax_at].iter())
            .copied()
            .collect()
    };
    let lower_half: Vec<_> = if xmin_at < xmax_at {
        polya[xmax_at..]
            .iter()
            .chain(polya[..=xmin_at].iter())
            .rev()
            .copied()
            .collect()
    } else {
        polya[xmax_at..=xmin_at].iter().rev().copied().collect()
    };
    ...
    for &(x, y) in &polyb {
        ...
        let p = upper_half.partition_point(|&(xi, _)| xi <= x);
        let (x0, y0) = upper_half[p - 1];
        let (x1, y1) = upper_half[p];
        if (y - y0) * (x1 - x0) >= (y1 - y0) * (x - x0) {
            return "NO";
        }
        let p = lower_half.partition_point(|&(xi, _)| xi <= x);
        let (x0, y0) = lower_half[p - 1];
        let (x1, y1) = lower_half[p];
        if (y - y0) * (x1 - x0) <= (y1 - y0) * (x - x0) {
            return "NO";
        }
    }
...
impl UnionFind {
    ...
    fn connect(&mut self, from: usize, to: usize) {
        let (ps, pt) = (self.find(from), self.find(to));
        if ps != pt {
            self.parent[ps] = pt;
        }
    }
