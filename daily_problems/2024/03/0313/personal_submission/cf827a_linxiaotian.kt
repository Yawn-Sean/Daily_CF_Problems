...
private fun PrintWriter.solve() {
    ...
    val indexesList = mutableListOf<List<Int>>()
    repeat(n) {
        strList.add(readString())
        val k = readInt()
        indexesList.add(readInts1(k).sorted())
    }
    println(stringReconstruction(strList, indexesList))
}

private fun stringReconstruction(strList: List<String>, indexesList: List<List<Int>>): String {
    val n = strList.size
    val indexToStr = mutableListOf<Pair<Int, String>>()
    var m = 0
    for (i in 0 until n) {
        m = m.max(indexesList[i].last() + strList[i].length)
        for (index in indexesList[i]) {
            indexToStr.add(Pair(index, strList[i]))
        }
    }
    // println(m)
    val queue = indexToStr.sortedBy { it.first }.toArrayDeque()
    val result = CharArray(m) { 'a' }
    var i = 0
    while (queue.isNotEmpty()) {
        val (index, str) = queue.removeFirst()
        i = i.max(index)
        // println("$i, $index, $str")
        for (j in max(i - index, 0) until str.length) {
            result[i++] = str[j]
        }
    }
