...
fn solver(n: usize) -> impl Printable {
    let mat = quick_pow([[7, 20], [0, 27]], n);
    mat[0][1]
}

#[inline]
fn mat_mul(lhs: [[usize; 2]; 2], rhs: [[usize; 2]; 2]) -> [[usize; 2]; 2] {
    [
        [
            (lhs[0][0] * rhs[0][0] + lhs[0][1] * rhs[1][0]) % MOD,
...
fn quick_pow(mut base: [[usize; 2]; 2], mut exp: usize) -> [[usize; 2]; 2] {
    let mut ret = [[1, 0], [0, 1]];
    while exp > 0 {
        if exp & 1 != 0 {
