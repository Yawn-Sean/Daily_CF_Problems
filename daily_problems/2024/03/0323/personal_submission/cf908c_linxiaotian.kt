...
private fun PrintWriter.printNums(nums: List<Double>) {
    if (nums.isEmpty()) {
        println()
        return
    }
...
private fun PrintWriter.solve() {
    val (n, r) = readInts()
    val xList = readDoubles(n)
    printNums(newYearCurling(r, xList))
}

private fun newYearCurling(r: Int, xList: List<Double>): List<Double> {
    val n = xList.size
    val rr4 = r * r * 4.0
    val r2 = r * 2.0
    ...
    for (i in 0 until n) {
        ...
        for (j in 0 until i) {
            val lastX = xList[j]
            val lastY = result[j]
            if (lastX < x - r2 || lastX > x + r2) {
                continue
            }
            val dx = x - lastX
            val newY = Math.sqrt(rr4 - dx * dx) + lastY
            y = y.max(newY)
        }
        result.add(y)
    }
