...
fn solver(n: usize, r: i64, scanner: &mut Scanner) -> impl Printable {
    ...
    for (i, &xi) in xs.iter().enumerate() {
        ...
        for (j, &xj) in xs[..i].iter().enumerate() {
            if xi + 2 * r < xj || xj + 2 * r < xi {
                continue;
            }
            let dy2 = d2 - (xi - xj) * (xi - xj);
            mx = mx.max(ans[j] + (dy2 as f64).sqrt());
        }
        ans.push(mx);
    }
    Sep::<_, ' '>(ans.into_iter().map(|x| format!("{}", x)))
}
