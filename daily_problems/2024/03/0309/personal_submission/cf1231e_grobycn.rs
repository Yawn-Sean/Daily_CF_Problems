...
fn solver(n: usize, s: String, t: String) -> impl Printable {
    ...
    t.bytes().for_each(|b| count[(b - b'a') as usize] -= 1);
    if count.iter().any(|&c| c != 0) {
        return Err(-1);
    }

    let mut ans = 0;
    // check if s contains a sequence matches pat
    let check_match_exact = |pat: &[u8]| {
        // O(len(pat) + n)
    ...
    // check if s contains a sequence that is a substring of t
    let bt = t.as_bytes();
    let check_match_substring = |len: usize| {
        // O((len + n) * (n - len + 1))
        for w in bt.windows(len) {
            if check_match_exact(w) {
                return true;
            }
        }
        false
