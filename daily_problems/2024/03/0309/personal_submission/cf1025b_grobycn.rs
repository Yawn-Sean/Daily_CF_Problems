...
fn solver(pairs: Vec<(usize, usize)>) -> impl Printable {
    ...
    let mut goal = 0; // gcd of all lcm(ai, bi)
    ...
    // choose any prime factor of goal
    // but goal may be too large
    for &(a, b) in pairs.iter() {
        for x in [a, b] {
            let g = gcd(x, goal);
            if g == 1 {
            ...
            // g is factor of goal and 2 <= g <= 2*10^9
            // find any prime factor of g
            for p in 2.. {
                if p * p > g {
                    break;
                }
