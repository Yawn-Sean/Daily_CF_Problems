...
fn solver(n: usize, l: usize, r: usize, mut s: usize) -> impl Printable {
    use std::collections::HashSet;

    let (l, r) = (l - 1, r - 1);
    let cnt = r + 1 - l;
    let min = (1 + cnt) * cnt / 2;
    let max = (n + n + 1 - cnt) * cnt / 2;
    if s < min || s > max {
        return Err(-1);
    }
    s -= min;
    let mut ans = vec![0; n];
    ans[l..=r].iter_mut().zip(1..).for_each(|(x, y)| *x = y);
    let (q, rem) = (s / cnt, s % cnt);
    ans[l..=r].iter_mut().for_each(|x| *x += q);
    ans[l..=r].iter_mut().rev().take(rem).for_each(|x| *x += 1);
    let seen = ans[l..=r].iter().copied().collect::<HashSet<_>>();
    ans.iter_mut()
        .filter(|x| **x == 0)
        .zip((1..=n).filter(|x| !seen.contains(x)))
        .for_each(|(x, y)| *x = y);
    Ok(Sep::<_, ' '>(ans))
}
