...
private fun PrintWriter.solve() {
    ...
    for (tx in 1 until t + 1) {
        // print("Case #$tx: ")
        val (n, l, r, s) = readInts(4)
        printNums(perSum(n, l, r, s))
    }
}

private fun perSum(n: Int, l: Int, r: Int, s: Int): List<Int> {
    val len = r - l + 1
    val min = (1 + len) * len / 2
    val max = (n - len + 1 + n) * len / 2
    if (s < min || s > max) {
        return listOf(-1)
    }
    val visited = BooleanArray(n) { it < len }
    var need = s - min
    var left = len - 1
    var right = n - 1
    while (need > 0) {
        val diff = min(need, right - left)
        visited[left] = false
        visited[left + diff] = true
        need -= diff
        left--
        right--
    }
    val result = IntArray(n)
    val selected = (0 until n).filter { visited[it] }.toArrayDeque()
    val others = (0 until n).filter { !visited[it] }.toArrayDeque()
    for (i in 0 until n) {
        result[i] = if (i >= l - 1 && i < r) selected.removeFirst() else others.removeFirst()
    }
