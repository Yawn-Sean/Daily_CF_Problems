...
private fun PrintWriter.solve() {
    ...
    val t = readInt()
    for (tx in 1 until t + 1) {
        // print("Case #$tx: ")
        val sum = readInt()
        println(sumToNum[sum])
    }
}

private val N = 10_000_000 + 1000
private val numToSum = IntArray(N) { -1 }
private val sumToNum = IntArray(N) { -1 }

private fun initMapping() {
    numToSum[1] = 1
    for (i in 2 until N) {
        if (Helper.maxFactor[i] == 0) {
            numToSum[i] = i + 1
            continue
        }
        var j = i
        numToSum[i] = 1
        while (j % Helper.maxFactor[i] == 0) {
            j /= Helper.maxFactor[i]
            numToSum[i] = numToSum[i] * Helper.maxFactor[i] + 1
        }
        numToSum[i] *= numToSum[j]
    }
    // println(numToSum[18])
    for (i in N - 1 downTo 0) {
        if (numToSum[i] != -1 && numToSum[i] < N) {
            sumToNum[numToSum[i]] = i
        }
    }
    // println("fin")
}
...
object Helper {
    ...
    private val MAX_NUM = N
    // maxFactor[i] == i 时，i 是质数
    val maxFactor = IntArray(MAX_NUM)
    val primeNums = mutableListOf<Int>()

    init {
        for (i in 2 until MAX_NUM) {
            if (maxFactor[i] != 0) {
                continue
            }
            for (j in i until MAX_NUM step i) {
                maxFactor[j] = i
            }
        }
        for (i in 2 until MAX_NUM) {
    ...
    fun isPrime(i: Int): Boolean {
        return maxFactor[i] == i
    }

    fun primeFactor(n: Int): List<Int> {
        val result = mutableListOf<Int>()
        var num = n
        while (num > 1) {
            val prime = maxFactor[num]
            while (num % prime == 0) {
                result.add(prime)
                num /= prime
            }
