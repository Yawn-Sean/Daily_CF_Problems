...
fn solver(n: usize, scanner: &mut Scanner) -> impl Printable {
    ...
    let mut factors = vec![vec![]; MAX + 1];
    for x in 1..=MAX {
        for y in (x..=MAX).step_by(x) {
            factors[y].push(x);
        }
    }
    let mut ret: Mod = Mod(0);
    for lcm in 1..=MAX {
        if presum[MAX] - presum[lcm - 1] == 0 {
            break;
        }
        let mut prod = Mod(1);
        for (w, k) in factors[lcm].windows(2).zip(1..) {
            prod = prod * power(Mod(k), presum[w[1] - 1] - presum[w[0] - 1]);
        }
        let cnt = presum[MAX] - presum[lcm - 1];
        let k = factors[lcm].len();
        prod = prod * (power(Mod(k), cnt) - power(Mod(k - 1), cnt));
        ret = ret + prod;
    }
    ret.0
}

fn power<const M: usize>(mut base: Mod<M>, mut exp: usize) -> Mod<M> {
    let mut ret = Mod(1);
    while exp > 0 {
        if exp & 1 != 0 {
            ret = ret * base;
