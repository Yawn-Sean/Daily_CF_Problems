...
private val N = 100_000 + 100

private fun elpLcm(nums: IntArray): Long {
    ...
    val factors = Array(N) { mutableListOf<Int>() }
    for (i in 1 until N) {
    ...
    var result = 0L
    for (i in 1 until N) {
        val k = factors[i].size
        if (counts[i] == 0 || k == 0) {
            continue
        }
        var current = 1L
        for (j in 1 until k) {
            val diff = counts[factors[i][j - 1]] - counts[factors[i][j]]
            // println("$i, $j | $diff, ${fastPow(j.toLong(), diff.toLong())}")
            current = current.modMul(fastPow(j.toLong(), diff.toLong()))
        }
        // println("${fastPow(k.toLong(), counts[i].toLong())}, ${fastPow(k.toLong() - 1L, counts[i].toLong())}")
        current = current.modMul(fastPow(k.toLong(), counts[i].toLong()) - fastPow(k - 1L, counts[i].toLong()))
        // println("# $i, $current")
        ...
        for (b in 1 until 3) {
            for (c in 1 until 4) {
                for (d in 1 until 5) {
                    ...
                    val l = nums.reduce { acc, i -> lcm(acc, i) }
                    if (l == nums.findMax() && l == 4) {
                        println(nums)
                    }
                }
            }
