...
private fun PrintWriter.solve() {
    ...
    println(prank(n))
    // println(prank(450))
    // println((0 until 100).map { prank(it + 1) })
}

private fun prank(n: Int): Long {
    val dp = Array(n + 1) { Array(n + 1) { LongArray(2) { -1L } } }.apply {
        search(0, 0, n, this)
    }
    return dp[0][0][1]
}
...
private fun search(
    ...
    if (dp[left][right].any { it >= 0L }) {
        return
    }
    val dpLeft = LongArray(2) { -1L }
    if (left < n) {
        search(left + 1, right, n, dp)
        dp[left + 1][right].copyInto(dpLeft)
    }
    val dpRight = LongArray(2) { -1L }
    if (left > right) {
        search(left, right + 1, n, dp)
        dp[left][right + 1].copyInto(dpRight)
    }
    dp[left][right][0] = dpLeft[1].max(0L) + dpRight[1].max(0L)
    dp[left][right][1] = dp[left][right][0]
    if (dpLeft[0] >= 0L && dpRight[0] >= 0L) {
        val current = max(dpLeft[0] + dpRight[1], dpLeft[1] + dpRight[0]) + 1L
        dp[left][right][1] = dp[left][right][1].max(current)
    } else if (dpLeft[0] >= 0L) {
        dp[left][right][1] = dp[left][right][1].max(dpLeft[0] + 1L)
    } else if (dpRight[0] >= 0L) {
        dp[left][right][1] = dp[left][right][1].max(dpRight[0] + 1L)
    }
    dp[left][right][0] %= MODL
    dp[left][right][1] %= MODL
}
