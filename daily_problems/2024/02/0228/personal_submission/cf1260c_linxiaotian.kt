...
private fun PrintWriter.solve() {
    ...
    for (tx in 1 until t + 1) {
        // print("Case #$tx: ")
        val (r, b, k) = readLongArray(3)
        println(if (infiniteFence(r, b, k)) "REBEL" else "OBEY")
    }
}

private fun infiniteFence(r: Long, b: Long, k: Long): Boolean {
    return gcdL(r, b) + min(r, b) * (k - 1L) < max(r, b)
}
