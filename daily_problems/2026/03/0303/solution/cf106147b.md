...
我们可以用二分查询一个数是否在一个集合中。我们先输入 $[1]$ 保证每个元素都可以被表示。接下来我们将集合中一半的数再放入查询的列表。这样，如果查询结果是 $1$ ，答案就在这个集合中，否则不在。我们就可以利用这件事进行二分。

而这样做，我们进行了 $\mathcal{O}(\log k)$ 次查询，而查询的集合的总长度是 $\mathcal{O}(k)$ ，控制的较小。
