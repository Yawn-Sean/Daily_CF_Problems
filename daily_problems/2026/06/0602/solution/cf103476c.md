...
但是，这会使得缺的那个数 $x_0$ 以及其 $p,p^2,\dots,p^a$ 倍还没走到。不妨设 $x_0$ 的左侧点为 $x_1$ ，则在 $x_1p,x_1p^2$ 之间，我们可以插入 $x_0p,x_0p^2$，这样形成了 $x_1p,x_0p,x_0p^2,x_1p^2$ ，就解决掉了 $x_0p,x_0p^2$ 的问题。类似地，可以解决 $x_0p^{2k-1},x_0p^{2k-1}$ ，只需在某一组 $x_1p^{2k-1},x_1p^{2k}$ 之间插入即可。这样不断插入两个元素，最后只剩下一个没有遍历到的元素。
