...
/*
first, consider the case where Alice win.
1 pile, odd win
2 piles, all even lose, else win
	because Alice can always change other situations to "all even"
so... winning case are: piles can't be all even

second, consider the case when Alice win most.
so we have to make even piles not neighbor.
...
void solve(){
	int n; cin>>n;
	const int even=n/2;
	Z ans=Fac::C(n-even+1,even);
	ans*=Fac::fc[n/2]*Fac::fc[n-n/2];
	cout<<ans.val()<<'\n';
}
