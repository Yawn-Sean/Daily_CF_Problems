...
namespace pos{ // example: single set, range sum
	using S=int;
	S op(S a,S b){ // new s = a op b
		return a+b;
	}
...
int main(){
	...
	vector<int>p(n+1);
	{
		pos::SGT sgt(vector<pos::S>(n+1,1),n);
		vector<int>a(n+1);
		for(int i=1;i<=n;++i){
			cin>>a[i];
		}
		for(int i=n;i>0;--i){
			auto g=[&](const pos::S x)->bool {
				return x<a[i];
			};
			p[i]=sgt.max_right(1,g)+1;
			dbg(i,a[i],p[i]);
			sgt.set(p[i],0);
		}
		dbg(p);
	}
	{
		def::SGT sgt(n);
		for(int i=1;i<=n;++i){
			int r=(p[i]!=1)?sgt.query(1,p[i]-1):0;
			sgt.set(p[i],r+1);
			#ifdef LOCAL
